...
public class SwerveTab extends ShuffleboardTabBase {
    ...
    public void createEntries() {
        ...
        m_FLCancoder = createNumberEntry("FL Cancoder", 0, new EntryInfo(0, 0));
        m_FLIntegrated = createNumberEntry("FL Integrated", 0, new EntryInfo(1, 0));
        m_FLVelocity = createNumberEntry("FL Velocity", 0, new EntryInfo(2,0));

        m_FRCancoder = createNumberEntry("FR Cancoder", 0, new EntryInfo(0, 1));
        m_FRIntegrated = createNumberEntry("FR Integrated", 0, new EntryInfo(1, 1));
        m_FRVelocity = createNumberEntry("FR Velocity", 0, new EntryInfo(2, 1));

        m_BLCancoder = createNumberEntry("BL Cancoder", 0, new EntryInfo(0, 2));
        m_BLIntegrated = createNumberEntry("BL Integrated", 0, new EntryInfo(1, 2));
        m_BLVelocity = createNumberEntry("BL Velocity", 0, new EntryInfo(2, 2));

        m_BRCancoder = createNumberEntry("BR Cancoder", 0, new EntryInfo(0, 3));
        m_BRIntegrated = createNumberEntry("BR Integrated", 0, new EntryInfo(1, 3));
        m_BRVelocity = createNumberEntry("BR Velocity", 0, new EntryInfo(2, 3));

        m_odometryX = createNumberEntry("Odometry X", 0, new EntryInfo(4, 0));
        m_odometryY = createNumberEntry("Odometry Y", 0, new EntryInfo(4, 1));
        m_odometryYaw = createNumberEntry("Odometry Yaw", 0, new EntryInfo(4, 2));

        m_gyroYaw = createEntry("Gyro Yaw", 0, BuiltInWidgets.kGyro, new EntryInfo(6, 0, 2, 2));
